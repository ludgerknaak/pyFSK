...
def CFSkgen(inpSig, fskFreq1, fskFreq2):
    ...
    timepd = np.arange(0, len(inpSig), 1/1.2)
...
def FSKdemod(FSKsig):
    ...
    while i < len(FSKsig):
        ...
        for x in range(1, len(ref_frame) - 1):
            ...
            if np.logical_and(
                ref_frame[x - 1] < 0, ref_frame[x + 1] > 0
            ) or np.logical_and(ref_frame[x - 1] > 0, ref_frame[x + 1] < 0):
